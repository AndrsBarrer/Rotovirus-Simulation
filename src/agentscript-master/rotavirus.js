...
export default class RotavirusModel extends Model {
  ...
  setupPatches() {
    ...
    offsets.forEach((offset) => {
      ...
      this.patches
        .patchRectXY(centerX + offset, centerY, 3, 50)
        .ask((p) => (p.isRoad = true));
      // Create horizontal roads
      this.patches
        .patchRectXY(centerX, centerY + offset, 50, 3)
        .ask((p) => (p.isRoad = true));
    });
  }
    ...
    if (turtle.state === "infected") {
      ...
      if (this.infectedTicksDuration <= turtle.infectedTicksCount) {
        if (mortalityRoll <= this.mortality) {
          this.finishHim(turtle); // FATALITY, rotavirus wins
  ...
  move(turtle) {
    ...
    if (turtle.state === "infected") {
      if (turtle.infectedTicksCount % 60 === 0) {
        turtle.shouldMove =
          this.spinRoulette() > this.probStillWhenSick ? true : false;
      }
    }
    ...
    if (turtle.shouldMove) {
      if (nextPatch && nextPatch.isRoad) {
        turtle.forward(
          turtle.state === "infected" ? this.speed * 0.25 : this.speed
        ); // Move forward only if it's a road
      } else {
